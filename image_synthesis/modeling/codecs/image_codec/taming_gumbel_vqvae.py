...
class TamingGumbelVQVAE(BaseCodec):
    ...
    def get_tokens(self, imgs, **kwargs):
        if self.full_to_quantize.device != self.device:
            self.full_to_quantize = self.full_to_quantize.to(self.device)
        imgs = self.preprocess(imgs)
        code = self.enc(imgs)
        if self.quantize_number != 0:
            code = self.full_to_quantize[code]
    ...
    def one_hot_decode(self, one_hot_seq):
        b = one_hot_seq.shape[0]
        h, w = self.dec.h, self.dec.w
        onehot = rearrange(one_hot_seq, 'b n (h w) -> b n h w', b=b, h=h, w=w).float()

        # go from 2887 embedding space to 8192
        onehot = einsum('b n h w, n d -> b d h w', onehot, F.one_hot(self.quantize_to_full).float())
        z = einsum('b n h w, n d -> b d h w', onehot, self.dec.quantize.embed.weight)
        z = self.dec.forward_emb(z)
...
class DummyContentCodec(BaseCodec):
    ...
    def decode(self, img_seq):
        return self.vq.decode(img_seq=img_seq)
