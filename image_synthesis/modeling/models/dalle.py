...
class DALLE(nn.Module):
    def __init__(
        ...
        *,
        ...
        window_size = (image_size[0] // content_codec_config['params']['token_shape'][0],
                       image_size[1] // content_codec_config['params']['token_shape'][1])
        self.mask_pooler = nn.MaxPool2d(window_size, window_size)
    ...
    def prepare_condition(self, batch, condition=None):
        ...
        if 'obj_mask' in batch:
            # print('hereeeeee')
            cond_['obj_mask'] = self.mask_pooler(batch['obj_mask'].float()).reshape(batch['obj_mask'].shape[0], -1)
        if 'roi_bbox' in batch:
            # print('hereeeeee')
            cond_['roi_bbox'] = batch['roi_bbox']
    ...
    def generate_content(
        ...
        *,
        ...
        content_token = batch.get('quantized_image')
