...
class Block(nn.Module):
    ...
    def forward(self, x, encoder_output, timestep, mask=None):    
        if self.attn_type == "selfcross":
            ...
            a, att = self.attn(self.ln1(x, timestep), encoder_output=None, mask=mask)
            x = x + a 
...
class SketchInpaintingTransformer(nn.Module):
    def __init__(
        self,
        condition_seq_len=49,
        n_layer=14,
        n_embd=1024,
        n_head=16,
        content_seq_len=1024,
    ...
    def forward(
        ...
        cont_emb = self.content_emb(input)
        emb = cont_emb
        emb = torch.cat([cond_emb, emb], 1)

        for block_idx in range(len(self.blocks)):   
            if self.use_checkpoint == False:
                emb, att_weight = self.blocks[block_idx](emb, None, t.cuda()) # B x (Ld+Lt) x D, B x (Ld+Lt) x (Ld+Lt)
            else:
                emb, att_weight = checkpoint(self.blocks[block_idx], emb, None, t.cuda())
        logits = self.to_logits(emb)[:, cond_emb.shape[1]:] # B x (Ld+Lt) x n
        out = rearrange(logits, 'b l c -> b c l')
        # print(out.shape)
        return out
