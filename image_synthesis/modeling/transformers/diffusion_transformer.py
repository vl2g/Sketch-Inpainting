...
def log_categorical(log_x_start, log_prob, obj_mask=None):
    if obj_mask is not None:
        return (log_x_start.exp() * log_prob).sum(dim=1)
    else:
        # weights = obj_mask.sum(-1)
        return ((log_x_start.exp() * log_prob)*obj_mask.unsqueeze(1)).sum(dim=1)
...
def gumbel_log_onehot_to_index(log_x):
    return F.gumbel_softmax(log_x, hard=True, dim=1)
...
class DiffusionTransformer(nn.Module):
    def __init__(
        ...
        *,
        ...
        # if lpips_net in ['vgg', 'alex']:
        #     print('-'*50)
        #     print(f'Using LPIPS loss with {lpips_net}')
        #     print('-'*50)
    ...
    def multinomial_kl(self, log_prob1, log_prob2, obj_mask=None):   # compute KL loss on log_prob
        if obj_mask is None:
            return (log_prob1.exp() * (log_prob1 - log_prob2)).sum(dim=1)

        else:
            # weights = obj_mask.sum(-1)
            x = (log_prob1.exp() * (log_prob1 - log_prob2)) * obj_mask.unsqueeze(1)
            return x.sum(dim=1)
        

    def q_pred_one_timestep(self, log_x_t, t, obj_mask=None):         # q(xt|xt_1)
        ...
        log_probs = obj_mask.unsqueeze(1) * log_probs + (1. - obj_mask.unsqueeze(1)) * log_x_t

        return log_probs

    def q_pred(self, log_x_start, t, obj_mask=None):           # q(xt|x0)
        ...
        # if obj_mask is not None:
        #     mask_indices = obj_mask.unsqueeze(2)
        #     log_probs[:,:-1,:] = torch.eye(log_probs.shape[1]-1).unsqueeze(0).to(log_probs.device) * (1. - mask_indices)\
        #         + mask_indices * log_probs[:, :-1, :]

        # print(log_x_start.shape)
        log_probs = obj_mask.unsqueeze(1) * log_probs + (1. - obj_mask.unsqueeze(1)) * log_x_start
        return log_probs

    def predict_start(self, log_x_t, cond_emb, t, obj_mask=None):          # p(x0|xt)
        ...
        log_pred = obj_mask.unsqueeze(1) * log_pred + (1. - obj_mask.unsqueeze(1)) * log_x_t

        return log_pred

    def q_posterior(self, log_x_start, log_x_t, t, obj_mask=None):            # p_theta(xt_1|xt) = sum(q(xt-1|xt,x0')*p(x0'))
        ...
        log_qt = self.q_pred(log_x_t, t, obj_mask=obj_mask)                                  # q(xt|x0)
        ...
        log_qt_one_timestep = self.q_pred_one_timestep(log_x_t, t, obj_mask=obj_mask)        # q(xt|xt_1)
        ...
        q = q - q_log_sum_exp
        log_EV_xtmin_given_xt_given_xstart = self.q_pred(q, t-1, obj_mask=obj_mask) + log_qt_one_timestep + q_log_sum_exp
        return torch.clamp(log_EV_xtmin_given_xt_given_xstart, -70, 0)

    def p_pred(self, log_x, cond_emb, t, obj_mask=None):             # if x0, first p(x0|xt), than sum(q(xt-1|xt,x0)*p(x0|xt))
        if self.parametrization == 'x0':
            log_x_recon = self.predict_start(log_x, cond_emb, t, obj_mask=obj_mask)
            log_model_pred = self.q_posterior(
                log_x_start=log_x_recon, log_x_t=log_x, t=t, obj_mask=obj_mask)
        elif self.parametrization == 'direct':
            log_model_pred = self.predict_start(log_x, cond_emb, t, obj_mask=obj_mask)
        else:
            raise ValueError
        return log_model_pred

    @torch.no_grad()
    def p_sample(self, log_x, cond_emb, t, obj_mask=None):               # sample q(xt-1) for next step from  xt, actually is p(xt-1|xt)
        model_log_prob = self.p_pred(log_x, cond_emb, t, obj_mask=obj_mask)
    ...
    def q_sample(self, log_x_start, t, obj_mask=None):                 # diffusion step, q(xt|x0) and sample xt
        log_EV_qxt_x0 = self.q_pred(log_x_start, t, obj_mask=obj_mask)
    ...
    def _train_loss(self, x, cond_emb, is_train=True, obj_mask=None, roi_bbox=None):                       # get the KL loss
        ...
        log_xt = self.q_sample(log_x_start=log_x_start, t=t, obj_mask=obj_mask)
        xt = log_onehot_to_index(log_xt)

        ############### go to p_theta function ###############
        log_x0_recon = self.predict_start(log_xt, cond_emb, t=t, obj_mask=obj_mask)            # P_theta(x0|xt)
        log_model_prob = self.q_posterior(log_x_start=log_x0_recon, log_x_t=log_xt, t=t, obj_mask=obj_mask)      # go through q(xt_1|xt,x0)
        ...
        for index in range(t.size()[0]):
            this_t = t[index].item()
            same_rate = (obj_mask[index]*(x0_recon[index] == x0_real[index])).sum().cpu()/x0_real.size()[1]
            self.diffusion_acc_list[this_t] = same_rate.item()*0.1 + self.diffusion_acc_list[this_t]*0.9
            same_rate = (obj_mask[index]*(xt_1_recon[index] == xt_recon[index])).sum().cpu()/xt_recon.size()[1]
            self.diffusion_keep_list[this_t] = same_rate.item()*0.1 + self.diffusion_keep_list[this_t]*0.9

        # compute log_true_prob now 
        log_true_prob = self.q_posterior(log_x_start=log_x_start, log_x_t=log_xt, t=t, obj_mask=obj_mask)
        kl = self.multinomial_kl(log_true_prob, log_model_prob, obj_mask=obj_mask)
        mask_region = (xt == self.num_classes-1).float()
        mask_weight = mask_region * self.mask_weight[0] + (1. - mask_region) * self.mask_weight[1]
        kl = kl * mask_weight
        kl = sum_except_batch(kl)

        decoder_nll = -log_categorical(log_x_start, log_model_prob, obj_mask=obj_mask)
        ...
        # if self.lpips_net in ['alex', 'vgg']:
        #     x0_real_img = self.content_codec.vq.decode(x0_real)
        #     x0_recon_img = self.content_codec.vq.one_hot_decode(gumbel_log_onehot_to_index(log_x0_recon[:,:-1,:]))

        #     x0_recon_img = x0_recon_img / 127.5 - 1
        #     x0_real_img = x0_real_img / 127.5 - 1
        #     x0_recon_img = roi_pool(x0_recon_img, output_size=128, boxes=torch.split(roi_bbox.float(), 1, dim=0))
        #     x0_real_img = roi_pool(x0_real_img, output_size=128, boxes=torch.split(roi_bbox.float(), 1, dim=0))

        #     lpips_loss = self.lpips_loss(x0_recon_img, x0_real_img).squeeze()
        #     vb_loss += (self.lpips_weight * lpips_loss)

        if self.auxiliary_loss_weight != 0 and is_train==True:
            kl_aux = self.multinomial_kl(log_x_start[:,:-1,:], log_x0_recon[:,:-1,:], obj_mask=obj_mask)
    ...
    def forward(
        ...
        # print(input['condition_token'].shape)
        # exit()
        ...
        if is_train == True:
            log_model_prob, loss = self._train_loss(sample_image, cond_emb, obj_mask=input.get('obj_mask'), roi_bbox=input.get('roi_bbox'))
    ...
    def sample(
        ...
        if content_token != None:
            # print('Content token present')
            sample_image = input['content_token'].type_as(input['content_token'])

        if self.condition_emb is not None:  # do this
            with torch.no_grad():
                cond_emb = self.condition_emb(input['condition_token']) # B x Ld x D   #256*1024
            # print('Condition embedding present')
            cond_emb = cond_emb.float()
        else: # share condition embeding with content
        ...
        if start_step == 0:
            sample_image[torch.where(obj_mask == 1)] = self.num_classes - 1
            log_z_start = index_to_log_onehot(sample_image, self.num_classes)
            start_step = self.num_timesteps
            log_z = log_z_start.clone()
            with torch.no_grad():
                for diffusion_index in range(start_step-1, -1, -1):
                    t = torch.full((batch_size,), diffusion_index, device=device, dtype=torch.long)
                    log_z = self.p_sample(log_z, cond_emb, t, obj_mask=obj_mask)     # log_z is log_onehot
                    log_z = obj_mask.unsqueeze(1) * log_z + (1. - obj_mask.unsqueeze(1)) * log_z_start
            ...
            with torch.no_grad():
                for diffusion_index in range(start_step-1, -1, -1):
                    t = torch.full((batch_size,), diffusion_index, device=device, dtype=torch.long)
                    log_z = self.p_sample(log_z, cond_emb, t, obj_mask=obj_mask)     # log_z is log_onehot
                    log_z = obj_mask.unsqueeze(1) * log_z + (1. - obj_mask.unsqueeze(1)) * log_x_start
    ...
    def sample_fast(
        ...
        assert start_step == 0
        sample_image[torch.where(obj_mask == 1)] = self.num_classes - 1
        log_z = index_to_log_onehot(sample_image, self.num_classes)
        start_step = self.num_timesteps
        with torch.no_grad():
            # skip_step = 1
            ...
            for diffusion_index in diffusion_list:
                ...
                t = torch.full((batch_size,), diffusion_index, device=device, dtype=torch.long)
                log_x_recon = self.predict_start(log_z, cond_emb, t, obj_mask=obj_mask)
                if diffusion_index > skip_step:
                    model_log_prob = self.q_posterior(log_x_start=log_x_recon, log_x_t=log_z, t=t-skip_step, obj_mask=obj_mask)
                else:
                    model_log_prob = self.q_posterior(log_x_start=log_x_recon, log_x_t=log_z, t=t, obj_mask=obj_mask)
