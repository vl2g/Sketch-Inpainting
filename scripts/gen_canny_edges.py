...
if __name__ == '__main__':
    base_dir = '../../data/train'
    out_dir = os.path.join(base_dir, 'obj_edge_maps')
    ...
    seg_files_pth = os.path.join(base_dir, 'sr_seg_bboxes')
    files = os.listdir(seg_files_pth)

    import tqdm
    for f in tqdm.tqdm(files):
        fname = f.split('.')[0]
        im = cv2.imread(os.path.join(seg_files_pth, f))
        im = 255 - cv2.Canny(im, 50, 100)

        cv2.imwrite(os.path.join(out_dir, f'{f.replace(".jpg", ".png")}'), im)
